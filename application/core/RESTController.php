...
abstract class RESTController extends Controller {
	...
	public function __construct() {
		...
		//$this->load->library('RESTFormat', NULL, 'formatObject');
		...
		{
			$this->request->body = $this->formatObject->factory($this->request->body, $this->request->format)->to_array();
	...
	public function response($data = array(), $http_code = null) {
		...
		else
		{
			...
			// If the format method exists, call and return the output in that format
			elseif (method_exists($this->formatObject, 'to_'.$this->response->format))
			{
				// Set the correct format header
				header('Content-Type: '.$this->_supported_formats[$this->response->format]);

				$output = $this->formatObject->factory($data)->{'to_'.$this->response->format}();
			}
	...
	protected function _detect_method() {
		...
		if ($this->config->item('enable_emulate_request'))
		{
			if ($this->input->post('method')) {
				$method = strtolower($this->input->post('method'));
			}
			elseif ($this->input->get('method')) {
